...
const roomCode = () => {
  ...
  let c;
  do { c = Array.from({ length: 5 }, () => A[crypto.randomInt(A.length)]).join(""); }
  while (games.has(c));
  return c;
};
const streetIdx = (s) => (s === null ? -1 : STREETS.indexOf(s) === -1 ? STREETS.length : STREETS.indexOf(s));
/** snapshot ordering: (hand, streetIdx, streetLog.length) — action count derived, never stored */
function seqOf(g) { return { hand: g.hand, street: streetIdx(g.street), actions: g.streetLog.length }; }
...
function seatPlayer(g, token, name, stack) {
  const seat = g.players.length ? Math.max(...g.players.map((p) => p.seat)) + 1 : 0;
  g.players.push({
    token, name, seat, stack,
    committedStreet: 0, committedHand: 0,
    folded: false, allIn: false, acted: false,
    inHand: false, connected: false, removed: false,
  });
  g.totalChips += stack;
  return seat;
}

const alive = (g) => g.players.filter((p) => !p.removed);
const inHand = (g) => alive(g).filter((p) => p.inHand && !p.folded);
const canAct = (g) => inHand(g).filter((p) => !p.allIn);
const bySeat = (g, s) => g.players.find((p) => p.seat === s && !p.removed);
const byToken = (g, t) => g.players.find((p) => p.token === t && !p.removed);

function nextSeat(g, from, filter) {
  const seats = alive(g).map((p) => p.seat).sort((a, b) => a - b);
...
function assertConservation(g) {
  const sum = alive(g).reduce((a, p) => a + p.stack + p.committedStreet, 0) + g.pot
    + (g.pots ? g.pots.reduce((a, q) => a + q.remaining, 0) : 0);
  if (sum !== g.totalChips) {
...
function startHand(g) {
  const ready = alive(g).filter((p) => p.stack > 0);
  if (ready.length < 2) throw "Need 2+ players with chips.";
  g.hand += 1;
  g.state = "hand";
  g.street = "preflop";
  g.pot = 0; g.pots = null; g.ritRuns = 1;
  g.streetLog = []; g.handLog = {}; g.notice = "";
  for (const p of alive(g)) {
    Object.assign(p, { committedStreet: 0, committedHand: 0, folded: false, allIn: false, acted: false });
    p.inHand = p.stack > 0;
  }
  g.buttonSeat = g.buttonSeat === -1
    ? ready[0].seat
    : nextSeat(g, g.buttonSeat, (p) => p.inHand);

  const headsUp = inHand(g).length === 2;
  const sbSeat = headsUp ? g.buttonSeat : nextSeat(g, g.buttonSeat, (p) => p.inHand);
  const bbSeat = nextSeat(g, sbSeat, (p) => p.inHand);
  ...
  let firstToAct = nextSeat(g, bbSeat, (p) => p.inHand && !p.allIn);

  // Single UTG straddle (posted blind, straddler acts last preflop)
  const utg = bySeat(g, nextSeat(g, bbSeat, (p) => p.inHand));
  if (g.config.straddleAllowed && g.straddleNextToken && utg &&
      utg.token === g.straddleNextToken && utg.stack > 0 && !headsUp) {
    ...
    firstToAct = nextSeat(g, utg.seat, (p) => p.inHand && !p.allIn);
  }
...
function post(g, p, amt, label) {
  ...
  p.stack -= pay; p.committedStreet += pay; p.committedHand += pay;
...
function applyAction(g, token, action) {
  ...
  switch (action.type) {
    ...
    case "raise": { // raise.to = total committed this street after raising (also covers opening bet)
      ...
      if (raiseSize < B.lastFullRaiseSize && !isAllIn)
        throw `Minimum raise is to ${minTo}.`;
      p.stack -= add; p.committedStreet = to; p.committedHand += add;
      ...
      B.currentBet = to;
      g.streetLog.push({
        seat: p.seat, name: p.name, act: B.currentBet === to && raiseSize === to ? "bet" : "raise",
        amt: to, allIn: p.allIn, short: !full,
      });
      break;
    }
    case "allin": {
      ...
      p.committedStreet = to; p.committedHand += p.stack; p.stack = 0;
...
function advanceTurn(g) {
  ...
  g.betting.toActSeat = nextSeat(g, g.betting.toActSeat,
    (p) => p.inHand && !p.folded && !p.allIn && !(p.acted && p.committedStreet === g.betting.currentBet));
}

function streetDone(g) {
  const actors = canAct(g);
  if (actors.length === 0) return true; // everyone all-in
  return actors.every((p) => p.acted && p.committedStreet === g.betting.currentBet);
}

function settleStreetIfDone(g, force = false) {
  if (!force && !streetDone(g)) return false;
  // sweep street commits into the pot
  for (const p of alive(g)) { g.pot += p.committedStreet; p.committedStreet = 0; }
  g.handLog[g.street] = g.streetLog; g.streetLog = [];
  ...
  g.betting = {
    ...
    toActSeat: nextSeat(g, g.buttonSeat, (p) => p.inHand && !p.folded && !p.allIn),
  };
  return true;
}
...
function computePots(g) {
  const contrib = alive(g).filter((p) => p.committedHand > 0)
    .map((p) => ({ seat: p.seat, amt: p.committedHand, eligible: p.inHand && !p.folded }));
  const levels = [...new Set(contrib.filter((c) => c.eligible).map((c) => c.amt))].sort((a, b) => a - b);
  const pots = [];
  let prev = 0;
  for (const level of levels) {
    let amount = 0;
    for (const c of contrib) amount += Math.max(0, Math.min(c.amt, level) - prev);
    const eligible = contrib.filter((c) => c.eligible && c.amt >= level).map((c) => c.seat);
  ...
  if (residue > 0 && pots.length) { pots[pots.length - 1].amount += residue; pots[pots.length - 1].remaining += residue; }
...
function awardPots(g, awards) {
  ...
  for (const a of awards) {
    ...
    for (const winners of a.runs) {
      let share = per + (leftover > 0 ? 1 : 0); if (leftover > 0) leftover--;
      ...
      for (const s of winners) {
        const w = bySeat(g, s);
        w.stack += each + (odd > 0 ? 1 : 0); if (odd > 0) odd--;
      }
    }
    pot.remaining = 0;
  }
...
function endHand(g) {
  ...
  g.streetLog = [];
  g.betting = null;
  for (const p of alive(g)) { p.inHand = false; p.folded = false; p.allIn = false; p.acted = false; p.committedHand = 0; }
}
...
function snapshot(g) {
  ...
  return {
    ...rest,
    players: players.map(({ ws, connected, ...p }) => p), // connection state is derived, never persisted
    unseated: unseated.map(({ ws, ...u }) => u),
  };
}
function publicState(g) {
  return {
    ...snapshot(g),
    players: alive(g).map((p) => ({ ...p, ws: undefined, token: undefined, connected: p.connected })),
    unseated: g.unseated.map((u) => ({ id: u.id, name: u.name })),
    seq: seqOf(g),
  };
}
...
function broadcast(g) {
  ...
  for (const p of alive(g)) {
    if (!p.ws) continue;
    send(p.ws, {
      type: "state", state, snapshot: snap, seq: state.seq,
      you: { seat: p.seat, isHost: p.token === g.hostToken },
    });
  }
  for (const u of g.unseated) send(u.ws, { type: "state", state, seq: state.seq, you: { seat: -1, isHost: false } });
}
...
wss.on("connection", (ws) => {
  ...
  ws.on("close", () => {
    ...
    if (pl && pl.ws === ws) { pl.ws = null; pl.connected = false; broadcast(g); }
    const u = g.unseated.find((x) => x.ws === ws);
    if (u) { g.unseated = g.unseated.filter((x) => x !== u); broadcast(g); }
  });
});

function requireHost(g, token) { if (token !== g.hostToken) throw "Host only."; }
...
function handle(ws, m) {
  ...
  if (m.type === "create") {
    ...
    const p = byToken(g, token); p.ws = ws; p.connected = true;
  ...
  if (m.type === "hello") {
    ...
    if (p) { p.ws = ws; p.connected = true; return broadcast(g); }
  ...
  switch (m.type) {
    ...
    /* ---- host controls ---- */
    case "start_hand": requireHost(g, token); startHand(g); break;
    case "set_rit": {
      requireHost(g, token);
      ...
      g.notice = g.ritRuns > 1
        ? `Running it ${g.ritRuns}× (all all-in players must agree at the table).` : "Single run.";
      break;
    }
    case "award": requireHost(g, token); awardPots(g, m.awards); break;
    case "force_fold": {
      requireHost(g, token);
    ...
    case "seat_player": { // seat someone from the join queue with a buy-in
      requireHost(g, token);
      if (g.state === "hand") throw "Seat players between hands.";
      const u = g.unseated.find((x) => x.id === m.uid);
      ...
      const np = bySeat(g, seat); np.ws = u.ws; np.connected = true;
      if (np.ws) np.ws.meta = { room: g.code, token: u.token };
      g.unseated = g.unseated.filter((x) => x !== u);
      break;
    }
    case "assign_seat": { // hand an existing seat (stack and all) to someone in the queue
      requireHost(g, token);
      const u = g.unseated.find((x) => x.id === m.uid);
      const p = bySeat(g, m.seat);
      ...
      p.token = u.token; p.name = u.name; p.ws = u.ws; p.connected = true;
    ...
    case "remove_player": {
      ...
      g.totalChips -= p.stack; p.stack = 0; p.removed = true; p.connected = false; p.ws = null;
      g.notice = `${p.name} left the game (stack cashed out).`;
      assertConservation(g);
      break;
    }
    case "add_chips": { // rebuy / top-up between hands
      requireHost(g, token);
...
function handleUnknownRoom(ws, room, m) {
  ...
  if (!p) { p = { first: Date.now(), best: null, sockets: new Set() }; pending.set(room, p); }
  p.sockets.add(ws);
}
...
function offerSnapshot(ws, m) {
  ...
  if (!validSnapshot(snap, room)) return send(ws, { type: "error", message: "Snapshot rejected (bad shape)." });
  const seq = { hand: snap.hand, street: streetIdx(snap.street), actions: (snap.streetLog || []).length };
...
function notifyRestorable(room) {
  ...
  for (const ws of p.sockets) {
    ...
    send(ws, {
      type: "restore_prompt",
      room, seq: p.best.seq, savedAt: p.best.savedAt, fromName: p.best.fromName,
      canRestore: isHost || anyoneMayRestore,
      youAreHost: isHost,
      waitMs: anyoneMayRestore ? 0 : RESTORE_HOST_WAIT_MS - (Date.now() - p.first),
    });
  }
  if (!anyoneMayRestore) setTimeout(() => notifyRestorable(room), RESTORE_HOST_WAIT_MS - (Date.now() - p.first) + 250);
}
...
function doRestore(ws, m) {
  ...
  if (!isHost && Date.now() - p.first <= RESTORE_HOST_WAIT_MS) throw "Waiting for the host to restore.";
  ...
  for (const s of p.sockets) {
    ...
    if (pl) { pl.ws = s; pl.connected = true; }
...
function validSnapshot(s, room) {
  ...
  const sum = s.players.filter((p) => !p.removed).reduce((a, q) => a + q.stack + q.committedStreet, 0)
    + s.pot + (s.pots ? s.pots.reduce((a, q) => a + (q.remaining ?? q.amount), 0) : 0);
...
function reviveSnapshot(s, room) {
  ...
  g.players = s.players.map((p) => ({ ...p, ws: null, connected: false })); // presence recomputed from live sockets
  g.streetLog = s.streetLog || []; g.handLog = s.handLog || {};
  return g;
}
...
setInterval(() => {
  ...
  for (const [code, g] of games)
    if (now - g.lastActivity > SWEEP_GAME_MS) games.delete(code);
  for (const [code, p] of pending)
    if (now - p.first > SWEEP_GAME_MS) pending.delete(code);

  if (!PUBLIC_URL) return;
  const active = [...games.values()].some(
    (g) => (g.state !== "idle" || alive(g).some((p) => p.connected)) && now - g.lastActivity < STALE_GAME_MS
  );
  if (active) fetch(`${PUBLIC_URL.replace(/\/$/, "")}/health`).catch(() => {});
}, 10 * 60 * 1000);

server.listen(PORT, () => {
  console.log(`Table Stakes on :${PORT}${PUBLIC_URL ? ` (keep-alive → ${PUBLIC_URL})` : " (no PUBLIC_URL: keep-alive off)"}`);
});
